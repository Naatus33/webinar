    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400">Acompanhe a performance dos seus webinars e leads.</p>
        </div>
        <button
          type="button"
          onClick={() => router.push("/webinar/new")}
          className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 px-6 text-sm font-medium text-white shadow-[0_0_20px_rgba(124,58,237,0.3)] transition-all hover:scale-105 hover:shadow-[0_0_30px_rgba(124,58,237,0.5)] focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 focus:ring-offset-slate-900"
        >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="flex flex-col gap-2 rounded-2xl border border-slate-800/60 bg-slate-900/50 p-5 backdrop-blur-sm">
          <div className="flex items-center gap-2 text-sm font-medium text-slate-400">
            <Users className="h-4 w-4 text-violet-400" />
            Total de Leads
          </div>
          <div className="text-3xl font-bold text-white">{totalLeads.toLocaleString("pt-BR")}</div>
        </div>
        <div className="flex flex-col gap-2 rounded-2xl border border-slate-800/60 bg-slate-900/50 p-5 backdrop-blur-sm">
          <div className="flex items-center gap-2 text-sm font-medium text-slate-400">
          ...
          <div className="text-3xl font-bold text-white">{activeWebinars}</div>
        </div>
        <div className="flex flex-col gap-2 rounded-2xl border border-slate-800/60 bg-slate-900/50 p-5 backdrop-blur-sm">
          <div className="flex items-center gap-2 text-sm font-medium text-slate-400">
          ...
          <div className="text-3xl font-bold text-white">{avgLeads}</div>
        </div>
      </div>

      <div className="space-y-4">
        {/* Controles de Lista: Busca e Filtros */}
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-xl border border-slate-800/50 bg-slate-900/20 p-2">
          ...
          <div className="relative w-full sm:max-w-xs px-1">
            ...
            <input
              ...
              className="h-9 w-full rounded-lg border border-slate-700/70 bg-slate-950/60 pl-9 pr-3 text-sm text-slate-50 outline-none placeholder:text-slate-500 transition focus:border-violet-500 focus:ring-1 focus:ring-violet-500"
            />
          </div>
        </div>
        ...
        {/* Lista de Webinars */}
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-800 py-20 text-center bg-slate-900/10">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-slate-800/50 mb-4">
              <Play className="h-6 w-6 text-slate-500 ml-1" />
            </div>
            ...
            {!search && activeFilter === "ALL" && (
              <button
                type="button"
                onClick={() => router.push("/webinar/new")}
                className="flex h-10 items-center rounded-xl bg-violet-600 px-6 text-sm font-medium text-white shadow-lg shadow-violet-500/20 transition hover:bg-violet-500"
              >
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-800/80 bg-slate-900/40 shadow-sm">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {filtered.map((w) => {
                  ...
                  return (
                    <tr
                      key={w.id}
                      className="group bg-transparent transition-colors hover:bg-slate-800/30"
                    >
                      <td className="px-5 py-4">
                        <div className="flex flex-col">
                          <span className="font-semibold text-slate-200 group-hover:text-white transition-colors">{w.name}</span>
                          <span className="text-xs text-slate-500 mt-0.5 truncate max-w-[200px] sm:max-w-xs">
                            /live/{w.code}/{w.slug}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4 text-slate-300">
                        {w.startDate ? (
                          <div className="flex flex-col gap-0.5">
                            ...
                            {w.startTime && (
                              <span className="text-xs text-slate-500 ml-5">{w.startTime}</span>
                            )}
                          </div>
                        ) : (
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          <div className="flex h-7 items-center justify-center rounded-lg bg-slate-800/80 px-2.5 text-xs font-medium text-slate-300 border border-slate-700/50">
                            <Users className="mr-1.5 h-3.5 w-3.5 text-violet-400" />
                            {w.leadsCount}
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-2 opacity-100 sm:opacity-60 transition-opacity group-hover:opacity-100">
                          ...
                          <Link
                            ...
                            title="Editar"
                            className="flex h-8 w-8 items-center justify-center rounded-lg bg-violet-500/10 text-violet-400 transition-colors hover:bg-violet-500/20 hover:text-violet-300"
                          >
